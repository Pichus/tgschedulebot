...
async def response(message: Message, state: FSMContext):
    ...
    async with chat_repository:
        chat_telegram_id = await chat_repository.get_chat_telegram_id_by_name_and_user_telegram_id(
            user_data["chosen_chat_name"], message.from_user.id)

    async with schedule_repository:
        await schedule_repository.upsert_schedule(chat_telegram_id, user_data["chosen_schedule_type"], message.text)
...
async def cmd_send_schedule(message: Message):
    ...
    async with chat_repository:
        ...
        schedule_repository = ScheduleRepository()
        async with schedule_repository:
            schedule = await schedule_repository.get_schedule(message.chat.id, current_week_type)


        bot_message = await message.answer(schedule)
        await chat_repository.add_schedule_message_to_edit_id(bot_message.message_id, message.chat.id)
