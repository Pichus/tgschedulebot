...
class ChatRepository(RepositoryBase):
    ...
    async def add_schedule_message_to_edit_id(self, schedule_message_id: int, chat_telegram_id: int):
        query = sql.SQL("UPDATE chats SET schedule_message_to_edit_id = %s WHERE chat_telegram_id = %s")
    ...
    async def get_chat_telegram_id_by_name_and_user_telegram_id(self, chat_name: str,
                                                                user_telegram_id: int) -> int:
        query = sql.SQL("""
